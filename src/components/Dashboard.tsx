...
export default function Dashboard() {
  ...
  // Sync internal state with URL brand
  useEffect(() => {
    setSelectedBrand(brandFromUrl);
  }, [brandFromUrl]);

  const fetchKPIData = useCallback(async (targetBrand: string) => {
    try {
      setLoading(true);
      const url = targetBrand === 'all' ? '/api/kpis' : `/api/kpis?brand=${targetBrand}`;
  ...
  useEffect(() => {
    fetchKPIData(brandFromUrl);
  }, [brandFromUrl, fetchKPIData]);
    ...
    return (
      <div className="min-h-screen bg-[#282929] text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            onClick={() => fetchKPIData(selectedBrand)}
